...
namespace Nop.Web.Controllers
{
    public partial class ProductController : BasePublicController
    {
        ...
        [NonAction]
        protected virtual ProductOverviewModel PrepareProductOverviewModel(Product product,
            bool preparePriceModel = true, bool preparePictureModel = true,
            int? productThumbPictureSize = null, bool prepareSpecificationAttributes = false,
            bool forceRedirectionAfterAddingToCart = false)
        {
            return this.PrepareProductOverviewModel(_workContext,
                _storeContext, _categoryService, _productService, _specificationAttributeService,
                _priceCalculationService, _priceFormatter, _permissionService,
                _localizationService, _taxService, _currencyService,
                _pictureService, _webHelper, _cacheManager,
                _catalogSettings, _mediaSettings, product,
                preparePriceModel, preparePictureModel,
                productThumbPictureSize, prepareSpecificationAttributes,
                forceRedirectionAfterAddingToCart);
        }
        ...
        [ChildActionOnly]
        public ActionResult RelatedProductsWithRecentlyViewedProduct(int productId, int? productThumbPictureSize)
        {
            //load and cache report
            var productIds = _cacheManager.Get(string.Format(ModelCacheEventConsumer.PRODUCTS_RELATED_IDS_KEY, productId, _storeContext.CurrentStore.Id),
                () =>
        ...
        [ChildActionOnly]
        public ActionResult RecentlyViewedProductWithRelatedProducts(int? productThumbPictureSize, bool? preparePriceModel)
        {
            if (!_catalogSettings.RecentlyViewedProductsEnabled)
                return Content("");
            ...
            //availability dates
            Product product = products.Where(p => p.IsAvailable()).FirstOrDefault();
            //prepare model
        ...
        public ActionResult CoolRss()
        {
            var feed = new SyndicationFeed(
                                    string.Format("{0}: Cool products", _storeContext.CurrentStore.GetLocalized(x => x.Name)),
                                    "Information about products",
                                    new Uri(_webHelper.GetStoreLocation(false)),
                                    "CoolRSS",
                                    DateTime.UtcNow);
        ...
        public ActionResult GiftsRss()
        {
            var feed = new SyndicationFeed(
                                    string.Format("{0}: Gift products", _storeContext.CurrentStore.GetLocalized(x => x.Name)),
                                    "Information about products",
                                    new Uri(_webHelper.GetStoreLocation(false)),
                                    "GiftsRSS",
                                    DateTime.UtcNow);
        ...
        [ChildActionOnly]
        public ActionResult HomepageProductsMarkAsNew(int? productThumbPictureSize)
        {
            var products = _productService.GetAllProductsMarkAsNewDisplayedOnHomePage();
            //ACL and store mapping
            products = products.Where(p => _aclService.Authorize(p) && _storeMappingService.Authorize(p)).ToList();
            //availability dates
            products = products.Where(p => p.IsAvailable()).ToList();
