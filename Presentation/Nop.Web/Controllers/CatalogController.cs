...
namespace Nop.Web.Controllers
{
    public partial class CatalogController : BasePublicController
    {
        ...
        [ChildActionOnly]
        public ActionResult RootCategorySelector()
        {
            //categories
            ...
            IList<CategoryModel> categoriesModel = new List<CategoryModel>();
            foreach(var cachedCategoryModel in cachedCategoriesModel)
            {
                CategoryModel categoryModel = new CategoryModel { Id = cachedCategoryModel.Id, Name = cachedCategoryModel.Name };
                categoriesModel.Add(categoryModel);
            }

            var model = new RootCategorySelectorModel { AvailableRootCategories = categoriesModel, CurrentCategoryId = categoriesModel[0].Id};
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        public ActionResult Deals(int categoryId, CatalogPagingFilteringModel command)
        {
            //get deals by categoryId
            var category = _categoryService.GetCategoryById(categoryId);
            if (category == null || category.Deleted)
                return InvokeHttp404();
            ...
            //all categories - not define key for CacheEvent ???
            string subCategoriesCacheKey = string.Format(ModelCacheEventConsumer.CATEGORY_SUBCATEGORIES_KEY,
                categoryId,
                string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                _storeContext.CurrentStore.Id,
                ...
                _webHelper.IsCurrentConnectionSecured());
            model.SubCategories = _cacheManager.Get(subCategoriesCacheKey, () =>
                _categoryService.GetAllCategoriesByParentCategoryId(0)  //get all categories
                .Select(x =>
                {
                    var subCatModel = new CategoryModel.SubCategoryModel
                    {
            ...
            //template
            int categoryTemplateId = 2;
            var templateCacheKey = string.Format(ModelCacheEventConsumer.DEALS_TEMPLATE_MODEL_KEY, categoryTemplateId);
            var templateViewPath = _cacheManager.Get(templateCacheKey, () =>
            {
                //var template = _categoryTemplateService.GetCategoryTemplateById(category.CategoryTemplateId);
                var template = _categoryTemplateService.GetCategoryTemplateById(categoryTemplateId);
                if (template == null)
                    template = _categoryTemplateService.GetAllCategoryTemplates().FirstOrDefault();
                if (template == null)
                    throw new Exception("No default deals template could be loaded");
                return template.ViewPath;
            });
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        public ActionResult PreSale(int categoryId, CatalogPagingFilteringModel command)
        {
            //get deals by categoryId
            var category = _categoryService.GetCategoryById(categoryId);
            if (category == null || category.Deleted)
        ...
        [NopHttpsRequirement(SslRequirement.No)]
        public ActionResult TopBrands(string allbrands, int categoryId, CatalogPagingFilteringModel command)
        {
            //Just test only. We will update later
            ...
            //template
            int ManufacturerTemplateId = 2;
            var templateCacheKey = string.Format(ModelCacheEventConsumer.TOPBRANDS_TEMPLATE_MODEL_KEY, ManufacturerTemplateId);
            var templateViewPath = _cacheManager.Get(templateCacheKey, () =>
            {
                var template = _manufacturerTemplateService.GetManufacturerTemplateById(ManufacturerTemplateId);
                if (template == null)
                    template = _manufacturerTemplateService.GetAllManufacturerTemplates().FirstOrDefault();
                if (template == null)
                    throw new Exception("No default top brands template could be loaded");
                return template.ViewPath;
            });
